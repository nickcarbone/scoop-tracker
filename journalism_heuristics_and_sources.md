...
### Added 2026-07-29 — entertainment/celebrity-desk markers

Prompted by adding Page Six/TMZ/Deadline/Variety as sources (see Sources section below)
and a real miss: the Suri Cruise name-change story (CBC's "docs obtained by" framing)
wasn't caught by the existing `documents? (obtained|reviewed) by` pattern because tabloid
copy almost universally says "docs," not "documents." Given explicitly less rigorous
testing per working-style discussion (regex against existing text, not a live-fetch
verification like sources require) — flagging that distinction here rather than implying
these got the same scrutiny as the source URL vetting below.

| Term | Category | Rationale |
|---|---|---|
| docs obtained by | sourcing_depth (broadened) | Broadened existing `documents? (obtained\|reviewed) by` to `(documents?\|docs) (obtained\|reviewed) by`. Tabloid-standard abbreviation, missed entirely before this — this was the literal phrase in the CBC Suri Cruise piece. |
| Exclusively obtained | exclusivity | Entertainment-desk variant of "newly obtained." |
| Source/insider tells | sourcing_depth | Standard tabloid direct-attribution construction ("a source tells Page Six," "an insider told Us Weekly"). Distinct from "people/sources familiar with" — that's third-person background sourcing on institutional stories; this is first-person quote attribution, the entertainment-beat equivalent. Scoped to "source/insider" + "tells/told" specifically to avoid matching routine reported speech. |
| Custody filing / Custody petition | primary_source | Family-court filing language, word order not caught by the existing generic "court filing[s]" pattern. |
| Divorce filing / Divorce papers | primary_source | Same logic — specific compound phrase, low false-positive risk. |
| Restraining order | primary_source | Specific legal instrument, genuine signal. |
| Petition filed | primary_source | Generic enough to also catch non-family-court petitions, but specific enough not to be noise. |
| Legal name change | institutional_record | Public-record filing category — this is the actual mechanism behind the Suri Cruise story. |
| Name change petition | institutional_record | More specific variant of the above — court petition, same rationale as PACER/SEC filing (text-mention now, could map to a future public-records cross-check later). |
...
### Discarded 2026-07-29 — entertainment-desk candidates

| Term | Why |
|---|---|
| Confirms to [Outlet] | Too generic — would match nearly any confirmed story regardless of actual sourcing depth (a rep confirming a routine announcement scores the same as a rep confirming a real scoop). No precision gain. |
| Insider reveals | "Reveals" is pure tabloid-headline editorializing, near-zero correlation with actual sourcing depth. |
| Shocking new details | Same issue — editorializing language, not a structural signal. |
...
### Added 2026-07-29 — entertainment/celebrity-desk sources

Prompted by a real miss: CBC's Suri Cruise/name-change coverage was flagged and scored, but
Page Six had reported the same story roughly a day earlier — Page Six wasn't a tracked source
at all, so the pipeline had no way to know CBC's write-up was a day-late follow, not the origin.
This is a coverage gap, not a clustering/representative-selection bug (see main.py's
`select_representative` docstring for the full explanation).

**Confirmed working (200, real feed content):** TMZ (`tmz.com/rss.xml`), Deadline
(`deadline.com/feed/`), Variety (`variety.com/feed/`).

**Sandbox-blocked here, added anyway (same `hostname_blocked` signature as NYT/Politico —
untested here, standard URL, likely fine in production):** Page Six (`pagesix.com/feed/`).

**Open scope question, not resolved here:** almost none of the existing Tier 1 heuristics
(FOIA, SEC filing, whistleblower, etc.) apply to celebrity/entertainment copy — most stories
from these four sources will ingest fine and score near zero until entertainment-specific
markers are added (see heuristics section above) and retuned against real output. Also worth
deciding deliberately, not by default: `PROJECT_INSTRUCTIONS.md` frames this tracker as
explicitly *not* a general aggregator — adding an entertainment beat is a real scope expansion,
not just four more RSS URLs, and should be revisited once there's a week of real output to
judge whether it's diluting the "must-read" feed or adding a genuine underserved beat.
